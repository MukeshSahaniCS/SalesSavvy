...
public class CartService {
	...
	public CartService(CartRepository crepo, UserRepository urepo, ProductRepository prepo,
			ProductImageRepository imagerepo) {
		this.crepo = crepo;
		this.urepo = urepo;
		this.prepo = prepo;
	...
	public void addToCart(int userId, int productId, int quanity) {
		...
		Users user = urepo.findById(userId)
				.orElseThrow(() -> new IllegalArgumentException("User not found with this UserId" + userId));
		Product product = prepo.findById(productId)
				.orElseThrow(() -> new IllegalArgumentException("Product not found with this productId" + productId));
	...
	public Map<String, Object> getCartItems(int userId) {
		...
		Users user = urepo.findById(userId)
				.orElseThrow(() -> new IllegalArgumentException("User not found with this UserId"));
